...
このリポジトリ（[nocore-dtm/JIZURA](https://github.com/nocore-dtm/JIZURA)）は、[852wa/JIZURA](https://github.com/852wa/JIZURA) のフォークです。本家の更新はそのまま取り込み、その上に次の機能を足しています。このサイトの実体は `main` ブランチ（root）です。

**▶ このフォークのブラウザ版：<https://nocore-dtm.github.io/JIZURA/>**

### カット単位で手法を選べる（詳細モードのみ）

- この節の機能は**詳細モード専用**です。**かんたん／スマホモードでは本家と同じ表示・挙動**に戻ります（カット情報は読むだけのチップ、カット単位の指定は使えません）。
- **行とカット**の一覧で、カットごとにレイアウトを選べるプルダウン（本家は行単位の指定）。
- タイムラインの**カット情報のチップ**（レイアウト・登場・保持・退場・カメラ・つなぎなど）を押すと、**そのカットだけ**の手法プレビューが開きます。サムネイルが自動でループするので、動きを見ながら差し替えられます。手で選んだチップは琥珀色で残ります。
- **このカットだけシャッフル / このカットだけおまかせ**。押したカットだけを引き直し、再生位置はその場に留まります。引き直しただけの項目は、手で選ぶまでチップを強調しません。
- カットへの指定は、本家の「統一感」「キメ」「くり返し（サビ）」より**あと**にかぶせます。統一感を使っていても指定したカットだけが差し変わり、指定していないカットの抽選結果は変わりません。

### 手法と演出のロック

- **演出**タブの各スライダー（動きの強さ・グリッチ・色ズレ・装飾の量・カットの細かさ・質感・背景の切替）と**フラッシュ**・**コマ打ち**、**手法**タブの**分類ごと**に鍵ボタンがあります。
- 鍵をかけた項目は**おまかせ / シャッフルでは変わりません**。スライダーなどは「いまの値」を、手法の分類は「いま ON になっている部品の組み合わせ」（例：カメラ 28/28、レイアウト 140/140）をそのまま保ちます。
- 鍵は **.jizura.json に保存**され、読み込むと復元されます。鍵が無い古いファイルは、鍵なしとして開きます。プロジェクトを開いた直後に ◀ で前のプロジェクトの鍵に戻ってしまわないよう、読み込み時に「元に戻す」の履歴を区切ります。

### 再生とプレビュー

- **ループボタン**が ループ（全体）→ 行ループ → カットループ → オフ の順に切り替わります。行ループはその行の全カット、カットループはそのカットだけをくり返します（タイムラインに琥珀色で範囲を表示）。
- **手法タブのカードが自動でループ再生**されます（ホバー不要。スマートフォンでも動きが分かります）。
- 再生位置がカットに移ると、開いている手法のプレビューも**そのカットに追従**します。

### タップ同期

- 「タップで同期」のボックスを**固定表示**にしました。行一覧の自動スクロールでボックスが流れ、押せなくなるのを防ぎます（狭い画面では画面に重ねて表示）。

この節は `main` ブランチにだけ入っています（本家へ出す `contrib` には入れません）。`main` に機能が入ったときは、この一覧も合わせて更新します。
