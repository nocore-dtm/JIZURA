...
このリポジトリ（[nocore-dtm/JIZURA](https://github.com/nocore-dtm/JIZURA)）は、[852wa/JIZURA](https://github.com/852wa/JIZURA) のフォークです。本家の更新はそのまま取り込み、このサイトの実体は `main` ブランチ（root）です。

**▶ このフォークのブラウザ版：<https://nocore-dtm.github.io/JIZURA/>**

### 本家に取り込まれたもの

このフォークで作った機能は、そのまま本家に入りました（本家 **v0.9.0** 時点）。

- **カット単位の手法差し替え** — [PR #23](https://github.com/852wa/JIZURA/pull/23)（本家 `bda33f2`）
  - カットごとのレイアウト指定、カット情報チップからの差し替え、このカットだけシャッフル／おまかせ、再生位置に追従するパネル（いずれも詳細モード限定）
- **ループ（全体／この行／このカット）** — [PR #22](https://github.com/852wa/JIZURA/pull/22)（本家 `e080a2a`）
- **手法と演出のロック** — [PR #24](https://github.com/852wa/JIZURA/pull/24)（本家 `752f556`）
  - 分類ごとの ON/OFF 固定（おまかせ／シャッフルで変わらない）、スライダー・フラッシュ・コマ打ちの値固定、`.jizura.json` への保存と復元
- **手法タブのカードのループプレビュー／タップ同期の固定** — [PR #18](https://github.com/852wa/JIZURA/pull/18)・[PR #19](https://github.com/852wa/JIZURA/pull/19)（内容は本家 `94413f7` で取り込み済み。PR はクローズ）

### いまこのフォークだけにあるもの

- **レイアウトの文字制御** — カット情報列のチップから、そのカットのレイアウトが**実際に描く文字**を一覧で出し、項目ごとに表示／非表示・書き換えができます（「本文」「行の残り（前後の語）」「通し番号」「時刻」「その他（レイアウト独自の文字）」に分類。詳細モード限定）。あわせて右のパネル（詳細モードの「演出」タブ／かんたんモード）に「通し番号」「時刻」の全体スイッチを追加しました（既定は ON、外すと全カットでその2項目を描きません。時刻は HUD のタイムコードも含む）。設定は `.jizura.json` に保存され、開き直しても復元されます。本家へ [PR #32](https://github.com/852wa/JIZURA/pull/32) として提出中です。
- それ以外のソースコードは**本家 v0.9.0 と同じ**です（上の機能ぶんを除く）。
- 本家と同じ 7 言語（日本語・English・繁體中文・简体中文・한국어・Bahasa Indonesia・Tiếng Việt）のページを、このフォークの Pages から配信している。
- `.gitignore` に `_probe.json` を 1 行足しているだけ（開発用のメモを無視するため）。
