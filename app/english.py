...
UI = {
  '`${i + 1}行目 カット${k + 1}の文字`': '`${i + 1}行目 カット${k + 1}の文字`',
  'このカットの文字（/ 区切りとして歌詞に書き戻します）': 'このカットの文字（/ 区切りとして歌詞に書き戻します）',
  'の文字': 'の文字',

  'の文字': 'の文字',

  'この行の頭とカットの頭をタップで打つ': 'この行の頭とカットの頭をタップで打つ',
  '行目のカットをタップ': '行目のカットをタップ',
  'このカットの開始（秒）': 'このカットの開始（秒）',
  'この行をループ再生します。少し前から再生するので、最初の 1 回は行の頭（カット1）を打ってください。以降は各カットの切り替わる瞬間に <span class="kbd">Tab</span>（または <span class="kbd">Space</span>）。押し間違えたら <span class="kbd">Backspace</span>、終了は <span class="kbd">Esc</span>。': 'この行をループ再生します。少し前から再生するので、最初の 1 回は行の頭（カット1）を打ってください。以降は各カットの切り替わる瞬間に <span class="kbd">Tab</span>（または <span class="kbd">Space</span>）。押し間違えたら <span class="kbd">Backspace</span>、終了は <span class="kbd">Esc</span>。',
  'この行はカットが1つです': 'この行はカットが1つです',
  'カットの同期が完了しました': 'カットの同期が完了しました',
